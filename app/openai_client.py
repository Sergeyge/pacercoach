...
_SYSTEM_PROMPT = (
    ...
    "and 'physiological_delta' (from this morning's recovery metrics). Only the second is a "
    "reason to reduce a session. A yellow day whose 'physiological_delta' is 0 or positive is "
    "yellow on VOLUME alone: the plan already decides how much to run, so the session stands "
    "exactly as planned — do not trim it, do not soften it, and never describe it as poor "
    "recovery. Reduce only when 'physiological_delta' is negative (or readiness is red), and "
    "note the bounds will already reflect that.\n"
